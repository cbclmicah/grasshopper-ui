...
define(['gh.core', 'moment', 'moment-timezone'], function(gh, moment, tz) {
    ...
    /**
     * Render the lecture view
     *
     * @param {Object}    terms    The terms containing the events per week
     * @private
     */
    var renderLectureView = function(terms) {
        // Add parent info to all events that have context
        addParentInfo();

        // Render the agenda view
        gh.utils.renderTemplate('lecturer-agenda-view', {
            'data': {
                'moment': moment,
                'openedTerms': require('gh.core').utils.localDataStorage().get('mylectures'),
                'terms': terms,
                'utils': gh.utils
            }
        }, $('#gh-my-lecture-view-container'));
    };
     ...
     * @private
     */
    var getLectureViewData = function(term, week) {
    ...
    var loadNextWeek = function() {
        ...
        getLectureViewData(term, nextWeek);
        // Send a tracking event
        gh.utils.trackEvent(['My lectures', 'Term', 'Load week'], {
            'term': term.name,
            'week': (nextWeek + 1)
        });
    };
     ...
     * @private
     */
    var refreshLectureView = function() {
        // Loop over all terms
        _.each(terms, function(term) {
            ...
            _.each(term.events, function(week, weekIndex) {
                // Get the events
                getLectureViewData(term, weekIndex);
            });
        });
    };
    ...
    var toggleTerm = function() {
        ...
        var expandedTerms = $('.lecture-view-term-header .fa-caret-down').map(function(index, value) {
        ...
        gh.utils.localDataStorage().store('mylectures', expandedTerms);
        // Send a tracking event
        gh.utils.trackEvent(['My lectures', 'Term', $(this).parent().next().is(':visible') ? 'Open' : 'Close'], {
    ...
    var addBinding = function() {
        ...
        $(document).on('shown.bs.tab', '#gh-calendar-view .gh-toolbar-primary a[data-toggle="tab"]', function(ev) {
            if ($(ev.target).attr('aria-controls') === 'gh-my-lecture-view') {
                // Load the first week of each term by default
                _.each(terms, function(term) {
                    getLectureViewData(term, 0);
                });

                // Send a tracking event
                gh.utils.trackEvent(['Tab', 'My lectures']);
            }
        });

        // Toggle the visiblity of a term when the header is clicked
        $(document).on('click', '.lecture-view-term-header > button', toggleTerm);

        // Load next week in term
        $(document).on('click', '.gh-btn-load-next-week', loadNextWeek);

        // Refresh the calendar
        $(document).on('gh.lectureview.refresh', refreshLectureView);
    };
